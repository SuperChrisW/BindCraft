...
def calculate_mpDockQ(pdb_fpath, target_chain="A", binder_chain='B', atom_distance_cutoff=4.0):
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure("complex", pdb_fpath)
    chain = structure[0][binder_chain]
    avg_bfactor = lambda x: np.mean([atom.get_bfactor() for atom in x]) if x else 0
    binder_plddt = np.array([avg_bfactor(res) for res in chain])

    binder_interface = hotspot_residues(pdb_fpath, target_chain=target_chain, binder_chain=binder_chain, atom_distance_cutoff=atom_distance_cutoff, only_ca=False)
    binder_inter_resIDs = [i-1 for i in list(binder_interface.keys())]
    
    avg_if_plddt = np.average(binder_plddt[binder_inter_resIDs])

    n_if_contacts = len(binder_inter_resIDs)
    x = avg_if_plddt*np.log10(n_if_contacts)
    pdockq_binder = 0.724 / (1 + np.exp(-0.052*(x-152.611)))+0.018

    #print({f"{id}_{res}":v for (id,res),v in zip(binder_interface.items(), binder_plddt[binder_inter_resIDs])})
    #print(f"mpDockQ score for binder: {pdockq_binder}")
    return pdockq_binder