...
class _af_prep:
  ...
  def _prep_partial_binder(self, pdb_filename,
      ...
      ---------------------------------------------------
      '''
      assert binder_chain is not None and binder_len is not None, "ERROR: please specify both binder_chain and binder_len"
      rm_binder = not kwargs.pop("use_binder_template", not rm_binder)
      
      self._args["use_sidechains"] = use_sidechains
      self._args["use_template"] = True
      
      # get pdb info
      chains = f"{target_chain},{binder_chain}"
      im = [ignore_missing] * len(target_chain.split(","))
      im += [ignore_missing] * len(binder_chain.split(","))

      self._target_pdb = prep_pdb(pdb_filename, chain=target_chain, ignore_missing=[ignore_missing] * len(target_chain.split(",")))
      self._binder_pdb = prep_pdb(pdb_filename, chain=binder_chain, ignore_missing=[ignore_missing] * len(binder_chain.split(",")))
      self._pdb = prep_pdb(pdb_filename, chain=chains, ignore_missing=im)

      self._target_len = sum([(self._pdb["idx"]["chain"] == c).sum() for c in target_chain.split(",")])
      self._binder_len = sum([(self._pdb["idx"]["chain"] == c).sum() for c in binder_chain.split(",")])
      self._new_binder_len = binder_len

      self._lengths = [self._target_len, self._new_binder_len]
      self._len = self._new_binder_len

      # gather hotspot info
      if hotspot is not None:
        self.opt["hotspot"] = prep_pos(hotspot, **self._target_pdb["idx"])["pos"]

      self.opt["weights"].update({"dgram_cce":1.0, "rmsd":0.0, "fape":0.0,
                                  "con":0.0, "i_con":0.0, "i_pae":0.0})

      # Set up position mapping for partial protocol
      if pos is None or not fix_pos:
        # Default to all binder positions
        self.opt["pos"] = None
        self._wt_aatype = self._binder_pdb["batch"]["aatype"]
        self._wt_aatype_sub = self._wt_aatype
      else:
        assert pos is not None, "ERROR: pos must be specified when fix_pos is True"

        self._pos_info = prep_pos(pos, **self._pdb["idx"]) # full sequence positions
        self.opt["pos"] = self._pos_info["pos"]
        self.opt['fix_pos'] = np.arange(self.opt["pos"].shape[0])
        assert all(self._target_len+self._binder_len > i >= self._target_len for i in self.opt["pos"]), "ERROR: fix_pos positions must be in binder sequence range"

        # Handle fix_pos for motif preservation
        self.opt["pos"] -= self._target_len # convert to binder sequence positions
        self._wt_aatype = self._binder_pdb["batch"]["aatype"]
        self._wt_aatype_sub = self._wt_aatype[self.opt["pos"]]

      # configure input features
      new_batch = {}
      for key in self._binder_pdb["batch"]:
        _new = np.full((self._target_len+binder_len, *self._binder_pdb["batch"][key].shape[1:]), 0) #[N, L, ...]
        _new[:self._target_len] = self._target_pdb["batch"][key]
        new_batch[key] = _new

      self._pdb["batch"] = new_batch
      res_idx = self._target_pdb["residue_index"]
      self._pdb["residue_index"] = np.concatenate([res_idx, res_idx[-1]+np.arange(self._new_binder_len)+50])

      self._inputs = self._prep_features(num_res=self._target_len+binder_len, num_seq=1)
      self._inputs["residue_index"] = self._pdb["residue_index"]
      self._inputs["batch"] = self._pdb["batch"]
      self._inputs.update(get_multi_id(self._lengths))
      self._binder_len = self._new_binder_len
