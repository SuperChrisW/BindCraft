...
from colabdesign.af.alphafold.common.residue_constants import restype_order
rev_restype_order = {v:k for k,v in restype_order.items()}

def seq_to_string(seq_onehot):
  '''convert one-hot sequence to amino acid string'''
  if seq_onehot.ndim == 3:
    # Handle batch dimension
    seq_indices = seq_onehot.argmax(-1)
    return ["".join([rev_restype_order.get(aa.item(), "X") for aa in seq]) for seq in seq_indices]
  else:
    # Handle single sequence
    seq_indices = seq_onehot.argmax(-1)
    return "".join([rev_restype_order.get(aa.item(), "X") for aa in seq_indices])
