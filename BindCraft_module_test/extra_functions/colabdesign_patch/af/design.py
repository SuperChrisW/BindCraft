...
class _af_design:
  ...
  def step(self, lr_scale=1.0, num_recycles=None,
    ...
    # Print current binder sequence if available
    if self.debug==True and self.protocol == "partial_binder" and "binder_seq_for_print" in self.aux:
      binder_seq = jax.device_get(self.aux["binder_seq_for_print"])
      binder_string = seq_to_string(binder_seq)

      plddt_mean = _reduce_mean_or_nan(self.aux.get("plddt", None))
      iptm_val   = _reduce_mean_or_nan(_safe_get_iptm(self.aux))
      now = time.time()
      json_payload = {
        "time_unix": now,
        "step": self._k,
        "binder_seq": binder_string,
        "plddt_mean": plddt_mean,
        "iptm": iptm_val,
        "pae_raw": _to_host_np(self.aux.get("pae", None)).tolist() if self.aux.get("pae", None) is not None else None,
        "plddt_raw": _to_host_np(self.aux.get("plddt", None)).tolist() if self.aux.get("plddt", None) is not None else None,
      }
      
      '''
      with open(f"/home/lwang/models/BindCraft/BindCraft_fork/IL23_pepBinder2/opt_log_l{len(binder_string)}.jsonl", "a") as jf:
          jf.write(json.dumps(json_payload) + "\n")
      '''
...
def _safe_get_iptm(aux):
    # BindCraft/AF2/AF3 variants may use 'i_ptm' or 'iptm'
    for k in ["i_ptm", "iptm", "iptm_score", "i_ptm_score"]:
        if k in aux:
            return aux[k]
    return None