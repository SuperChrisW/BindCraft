...
from colabdesign.af.prep import get_sc_pos
...
class design_model:
  ...
  def rewire(self, order=None, offset=0, loops=0):
    ...
    -----------------------------------------
    '''
    self.opt['native_binder_pos'] = self.opt["pos"]
    self.opt["pos"] = rewire(length=self._pos_info["length"], order=order,
                             offset=offset, loops=loops)

    # ============ partial_binder: add motif template positions ============
    if self.protocol == 'partial_binder':
      native_fix_pos = self.opt["native_binder_pos"] # binder sequence positions // not been rewired
      rewired_fix_pos = self.opt["pos"]+self._target_len # full sequence positions // have been rewired
      print(f"rewired_fix_pos: {rewired_fix_pos}, native_fix_pos: {native_fix_pos}")
      # add binder fix position template informations
      for k, v in self._inputs["batch"].items(): # inputs['batch'] has full shape of target + binder 
        native_value = self._binder_pdb['batch'][k]
        v[rewired_fix_pos,...] = native_value[native_fix_pos,...]

      # re-set remove template mask
      rm_keys = ["rm_template","rm_template_seq","rm_template_sc"]
      for k in rm_keys:
        if k in self._inputs:
          self._inputs[k][rewired_fix_pos] = False
      
      # re-set side chain
      def sub(x, pos, axis=0):
        return jax.tree_map(lambda y:jnp.take(y,pos,axis),x)

      atoms_to_exclude = None
      if self._args["use_sidechains"]:
        self._sc = {"batch":prep_inputs.make_atom14_positions(sub(self._inputs["batch"], pos=rewired_fix_pos, axis=0)),
                    "pos":get_sc_pos(self._wt_aatype[self.opt["pos"]], atoms_to_exclude)}
