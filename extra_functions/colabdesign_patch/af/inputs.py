...
from colabdesign.af.utils import seq_to_string
...
class _af_inputs:
  ...
  def _get_seq(self, inputs, aux, key=None):
    ...
    # Apply sequence fixing for non-partial_binder protocols
    if self.protocol != "partial_binder":
      seq = self._fix_pos(seq)
    else:
      seq = self._fix_binder_pos(seq)
      
    aux.update({"seq":seq, "seq_pseudo":seq["pseudo"]})
    
    # protocol specific modifications to seq features
    if self.protocol in ["binder", "partial_binder"]:
      ...
      # Store binder sequence for printing outside JIT
      aux["binder_seq_for_print"] = aux['seq_pseudo']
      aux["target_seq_for_print"] = seq_target
  ...
  def _fix_binder_pos(self, seq, return_p=False):
      if "fix_pos" in self.opt:
        if "pos" in self.opt:
          native_fix_pos = self.opt["native_binder_pos"] # binder sequence positions // not been rewired
          rewired_fix_pos = self.opt["pos"] # binder sequence positions // have been rewired
          seq_ref = jax.nn.one_hot(self._wt_aatype, self._args["alphabet_size"])
          
          def fix_seq(x):
            return x.at[..., rewired_fix_pos, :].set(seq_ref[..., native_fix_pos, :])
          seq = jax.tree_map(fix_seq, seq)

          if return_p: 
            return seq, rewired_fix_pos+self._target_len
        else:
          raise ValueError("pos not in self.opt")
      return seq

  def _update_template(self, inputs, key):
    ''''dynamically update template features''' 
    if "batch" in inputs:
      # in partial binder protocol, opt['pos'] is rewired position index in binder sequence
      ...
      # TODO: add template features for partial binder
      ...
      elif self.protocol == "partial_binder":
      ...
      for k,v in template_feats.items():
        if self.protocol == "partial":
          ...
          inputs[k] = inputs[k].at[0,:self._target_len].set(v[:self._target_len])
          inputs[k] = inputs[k].at[0,pos,...].set(v[pos,...])
        else:
